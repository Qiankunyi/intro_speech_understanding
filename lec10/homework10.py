...
def get_features(waveform, Fs):
    ...
    mstft = np.abs(np.fft.fft(
        np.array([waveform[m+1:m+1+L] - waveform[m:m+L]
    ...
    energy = np.sum(np.square(
        np.array([waveform[m:m+framelength]
    ...
    for num in range(len(startframes)):
        labels[5*startframes[num] : 5*endframes[num]+4] = num + 1
...
def train_neuralnet(features, labels, iterations):
    ...
    model = torch.nn.Sequential(
        torch.nn.LayerNorm(NFEATS, dtype=torch.float64),
        torch.nn.Linear(NFEATS, NLABELS, dtype=torch.float64)
    )
